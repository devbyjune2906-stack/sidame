...
const JENIS_LABEL: Record<string, string> = { DMED_PODI: "POD I", DMED_PI10: "PI 10%" };
...
export default async function DmedTPage() {
  ...
  const rows = await db
    ...
    .where(eq(processTemplate.subpokja, "DMED-T"))
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-line/60 last:border-0 hover:bg-sand/60">
                ...
                <td className="px-4 py-3 text-ink">{JENIS_LABEL[r.templateId] ?? r.templateId}</td>
