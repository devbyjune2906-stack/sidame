...
import { isAdmin, isDmee, canWrite } from "@/lib/rbac";
...
export default async function DmeePage() {
  ...
  const rows = await db
    ...
    .where(eq(wilayahKerja.statusWk, "EKSPLORASI"))
    .orderBy(asc(wilayahKerja.namaWk));

  return (
    <div className="space-y-5">
      <header>
        <h1 className="font-display text-2xl font-bold text-ink">WK Eksplorasi (DMEE)</h1>
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-line/60 last:border-0 hover:bg-sand/60">
                ...
                <td className="px-4 py-3">
                  <WkActionButtons id={r.id} editHref={`/wk/dmee/${r.id}/edit?back=/wk/dmee`} canWrite={userCanWrite} />
                </td>
              </tr>
            ))}
          </tbody>
