...
export default async function DmenPage() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="font-display text-2xl font-bold text-ink">WK Non Konvensional (DMEN)</h1>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={9} className="px-4 py-10 text-center text-muted">
                  Belum ada data WK Non Konvensional.
                </td>
              </tr>
            )}
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-line/60 last:border-0 hover:bg-sand/60">
                ...
                <td className="px-4 py-3">
                  <WkActionButtons id={r.id} editHref={`/wk/${r.id}/edit?back=/wk/dmen`} canWrite={userCanWrite} />
                </td>
              </tr>
            ))}
          </tbody>
