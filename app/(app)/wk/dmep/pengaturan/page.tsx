...
import { addDmepField, deleteDmepField } from "./actions";

const TIPE_LABEL: Record<string, string> = { text: "Teks", number: "Angka", date: "Tanggal" };

export default async function DmepPengaturanPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-ink">Kelola Kolom DMEP</h1>
          <p className="mt-1 text-sm text-muted">Kolom tambahan ini muncul di semua WK DMEP-L dan DMEP-P</p>
        </div>
        <Link href="/wk/dmep-l" className="text-sm text-petroleum hover:underline">Kembali</Link>
      </header>

      <Card>
        <h2 className="mb-4 text-base font-semibold text-ink">Tambah Kolom Baru</h2>
        <form action={addDmepField} className="space-y-3">
