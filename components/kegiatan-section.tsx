...
import { addKegiatanBaris, deleteKegiatan, deleteKegiatanBaris } from "@/app/(app)/wk/kegiatan-actions";
...
export function KegiatanSection({ id, judul, kolom, baris, subpokja, canEdit }: Props) {
  ...
  function handleTambahBaris() {
    startTransition(async () => {
      await addKegiatanBaris(id, subpokja, rowData, baris.length);
      setRowData({});
      setShowForm(false);
      router.refresh();
    });
  ...
  function handleHapusBaris(barisId: string) {
    startTransition(async () => {
      await deleteKegiatanBaris(barisId, subpokja);
      router.refresh();
    });
  }
  ...
  function handleHapusKegiatan() {
    ...
    startTransition(async () => {
      await deleteKegiatan(id, subpokja);
      router.refresh();
    });
  }
        ...
        <table className="w-full whitespace-nowrap text-sm">
          <thead>
            <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-muted">
              ...
              {canEdit && <th className="px-3 py-3 text-right font-semibold">Aksi</th>}
            </tr>
          </thead>
          <tbody>
            ...
            {baris.map((b) => (
              <tr key={b.id} className="border-b border-line/60 last:border-0 hover:bg-sand/60">
                {kolom.map((k) => (
                  <td key={k} className="px-3 py-3 text-ink">{b.data[k] ?? "—"}</td>
                ))}
                {canEdit && (
              ...
              <tr className="border-b border-petroleum/20 bg-petroleum/5">
                {kolom.map((k) => (
                  <td key={k} className="px-2 py-2">
                    <input
                      value={rowData[k] ?? ""}
                      onChange={(e) => setRowData((prev) => ({ ...prev, [k]: e.target.value }))}
                      placeholder={k}
                ...
                {canEdit && (
                  <td className="px-2 py-2">
                    <div className="flex justify-end gap-1.5">
                      ...
                      <button
                        onClick={() => { setShowForm(false); setRowData({}); }}
